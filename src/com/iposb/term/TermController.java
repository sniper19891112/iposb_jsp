...
public class TermController extends HttpServlet {
	...
	public static final String CPTERM_EDIT = "cpterm_edit"; // 導到編輯頁
	public static final String CPTERM_UPDATE = "cpterm_update"; // 更新資料庫

	private static String RESULT = "";

	static Logger logger = Logger.getLogger(TermController.class);
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (actionType == null || actionType.equals("")) {
			...
			data = TermBusinessModel.viewTerm();

			if (data != null && !data.isEmpty()) {
				request.setAttribute(OBJECTDATA, data);
			}

			logger.info("---User is reading Terms Of Service ("
					+ UtilsBusinessModel.timeNow() + ")");
			String url = "./terms.jsp";

			RequestDispatcher dispatcher = request.getRequestDispatcher(url);
			dispatcher.forward(request, response);
			return;

		} else if (actionType.equals(CPTERM_EDIT) == true) {
			...
			ArrayList<TermDataModel> data = new ArrayList<TermDataModel>();
			data = TermBusinessModel.termDetails();
			...
			if (!RESULT.equals("")) {
				TermDataModel obj = (TermDataModel) data.get(0);
			...
			String url = "./cp/cpTerm.jsp?actionType=cpterm_update";
			RequestDispatcher dispatcher = request.getRequestDispatcher(url);
			dispatcher.forward(request, response);
			return;

		} else if (actionType.equals(CPTERM_UPDATE) == true) {
			...
			ArrayList<TermDataModel> data = new ArrayList<TermDataModel>();
			data = TermBusinessModel.setRequestData(request, userId);

			String returnValue = "";
			String result = "";
			returnValue = TermBusinessModel.updateTerm(data); // 寫入資料庫
	...
	private boolean checkPrivilege(HttpServletResponse response, int privilege) throws IOException {
		...
		String url = "./stafflogin?returnUrl=./cpTerm";
