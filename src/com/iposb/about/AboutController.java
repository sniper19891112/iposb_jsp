...
public class AboutController extends HttpServlet {
	...
	public static final String CPABOUTUS_EDIT = "cpaboutUs_edit"; // 導到編輯頁
	public static final String CPABOUTUS_UPDATE = "cpaboutUs_update"; // 更新資料庫
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (actionType == null || actionType.equals("")) {
			...
			data = AboutBusinessModel.viewAboutUs();

			if (data != null && !data.isEmpty()) {
				request.setAttribute(OBJECTDATA, data);
			}

			logger.info("---User is reading About Us ("
		...
		} else if (actionType.equals(CPABOUTUS_EDIT) == true) {
			...
			ArrayList<AboutDataModel> data = new ArrayList<AboutDataModel>();
			data = AboutBusinessModel.aboutUsDetails();
			...
			if (!RESULT.equals("")) {
				AboutDataModel obj = (AboutDataModel) data.get(0);
			...
			String url = "./cp/cpAboutUs.jsp?actionType=cpaboutUs_update";
			RequestDispatcher dispatcher = request.getRequestDispatcher(url);
			dispatcher.forward(request, response);
			return;

		} else if (actionType.equals(CPABOUTUS_UPDATE) == true) {
			...
			ArrayList<AboutDataModel> data = new ArrayList<AboutDataModel>();
			data = AboutBusinessModel.setRequestData(request, userId);

			String returnValue = "";
			String result = "";
			returnValue = AboutBusinessModel.updateAboutUs(data); // 寫入資料庫
	...
	private boolean checkPrivilege(HttpServletResponse response, int privilege) throws IOException {
		int iid = 15; // 必須自行手動設定，對應DB的privilege
		String url = "./stafflogin?returnUrl=./cpAbout";
