...
<%		
	...
	if(source.equals("edit")) {
		...
		partnerId = joinData.get("partnerId") == null ? "" : joinData.get("partnerId").toString();
							...
							<div class="col-xs-12">
								<!-- PAGE CONTENT BEGINS -->
									...
									<form id="createForm" name="createForm" method="post" action="./consignment" class="form-horizontal">
										<input type="hidden" id="actionType" name="actionType" value="createJoinConsignment">
										...
										<div class="form-group">
											<label class="col-sm-4 control-label no-padding-right"> <%=Resource.getString("ID_LABEL_NETWORKPARTNER",locale)%> </label>
	
											<div class="col-sm-8">												
												<select class="col-xs-5" id="partnerId" name="partnerId">
					                                ...
					                                %>
														<option value="<%=pData.getPid() %>"><%=pData.getCname() %></option>
													<% } } %>
												</select>												
											</div>
										</div>
