<%@page import="com.iposb.i18n.*,com.iposb.logon.*,com.iposb.howitworks.*,com.iposb.privilege.*,java.util.ArrayList"%>
...
<%
	...
	if(data != null && data.size() > 0){
		...
		errmsg = howitworksData.getErrmsg();
	}
...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title><%=Resource.getString("ID_LABEL_HOWITWORKSMAINTENANCE",locale)%></title>
				...
				<div class="main-content">
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							...
							<li class="active"><%=Resource.getString("ID_LABEL_HOWITWORKSMAINTENANCE",locale)%></li>
						</ul><!-- .breadcrumb -->
					...
					<div class="page-content">
						<div class="row">
							...
							<form id="howitworksForm" name="howitworksForm" class="sky-form" method="post" action="./howitworks">
								<input type="hidden" id="actionType" name="actionType" value="<%=actionType %>">
								
								<%=Resource.getString("ID_LABEL_CONTENTEN",locale)%> <%=Resource.getString("ID_LABEL_COLON",locale)%>
								<textarea class="ckeditor" id="content_enUS"  name="content_enUS" cols="50" rows="6"><%=howitworksData.getContent_enUS() %></textarea>
								<br /><br />
								<!-- 
								<%=Resource.getString("ID_LABEL_CONTENTSC",locale)%> <%=Resource.getString("ID_LABEL_COLON",locale)%>
								<textarea class="ckeditor" id="content_zhCN" name="content_zhCN" cols="50" rows="6"><%=howitworksData.getContent_zhCN() %></textarea>
								<br /><br />
								
								<%=Resource.getString("ID_LABEL_CONTENTTW",locale)%> <%=Resource.getString("ID_LABEL_COLON",locale)%>
								<textarea class="ckeditor" id="content_zhTW" name="content_zhTW" cols="50" rows="6"><%=howitworksData.getContent_zhTW() %></textarea>
								<br /><br />
								-->
