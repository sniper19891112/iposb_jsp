<%@page import="com.iposb.i18n.*,com.iposb.logon.*,com.iposb.privilege.*,com.iposb.area.*,com.iposb.account.*,java.util.ArrayList,java.util.Date,java.text.*"%>
<%@include file="../include.jsp" %>

<%		
	if(priv < 9){
    	String url = "./stafflogin?returnUrl=./settlement";
	...
	String eventString = AccountBusinessModel.getSettlementRecordString4Calendar(year, month, station);
%>
...
<html lang="en">
	<head>
		<meta charset="utf-8" />
		<title><%=Resource.getString("ID_LABEL_SETTLEMENT",locale)%></title>
		...
		<script type="text/javascript" src="./assets/js/calgen2.js"></script>
	...
	<body>
				...
				<div class="main-content">
					...
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							...
							<li class="active"><%=Resource.getString("ID_LABEL_ACCOUNT",locale)%></li>
							<li class="active"><%=Resource.getString("ID_LABEL_SETTLEMENT",locale)%></li>
						...
						<div class="row">
							<div class="col-xs-12">
								<!-- PAGE CONTENT BEGINS -->
										...
										<div>
											<form class="form-inline" id="genCalForm" name="genCalForm">
												...
												<button id="submitBtn" class="btn btn-purple btn-sm">
													<%=Resource.getString("ID_LABEL_SHOWSETTLEMENT",locale)%>
									...
									<span id="generateCalHere"><div align="center"><i class="fa fa-circle-o-notch fa-spin"></i></div></span>
		...
		<script language="javascript" type="text/javascript">
			...
			$(document).ready(function () { 
				...
				var settlementDT;
				//upload documents
				$('[id^=callUploadImageBtn_]').on('click', '', function() {

					var thisID = this.id;
					settlementDT = thisID.substring(thisID.indexOf("_")+1, thisID.length);
					$("#thisDT").html(settlementDT);
					$("#uploadSuccess").hide();
					$("#uploadError").hide();
					$("#amount").val("");
					$("#remark").val("");
				...
				$('#upImg').change(function(){
					...
					$("#uploadImageModal #imageForm").attr("action", "./upload?actionType=accountSlip" + "&settlementDT=" + settlementDT + "&amount=" + amount + "&station=" + station + "&remark=" + remark);
					$("#uploadSuccess").hide();
					$("#uploadError").hide();
					$("#uploadImageBtn").hide();
					$("#closeBtn").hide();
					$("#uploadImageModal #loading").show();
					$("#uploadImageModal #loading").html("<i><%=Resource.getString("ID_LABEL_UPLOADING",locale) %>...</i>&nbsp;&nbsp;&nbsp;<i class='fa fa fa-spinner fa-spin btn-lg'></i>&nbsp;&nbsp;&nbsp;");
					$("#uploadImageModal #imageForm").ajaxForm({
						 success: function(data, status){
							...
							if(isTrue == true){
								...
								$('#bankslip_'+settlementDT).html("<a href='https://static.iposb.com/accountSlip/"+data+"' title='RM "+amount+" - "+remark+"' target='_blank' style='text-decoration:none; color: #FFFFFF; font-size:13px'><div style='background-color:#8CBF40; height:22px;'>&nbsp;&bull;&nbsp;RM " + amount +" - "+remark+"</div></a>");
							}else{
								$("#uploadImageModal #loading").hide();
								$("#uploadImageModal #loading").html("");
								$("#uploadImageBtn").show();
								$("#closeBtn").show();
								$("#uploadError").show();
								$("#uploadError").html("<%=Resource.getString("ID_LABEL_UPLOADFAILED",locale)%><%=Resource.getString("ID_LABEL_COLON",locale)%>" + data);
							};
					     }
					}).submit();
				});
			...
			function view(){
				...
				var M = $("#month").val();
				var S = $("#station").val();
				window.location.href="./settlement?year="+Y+"&month="+M+"&station="+S;
			};
